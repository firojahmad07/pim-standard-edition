...
namespace Akeneo\Pim\Tailored\Infrastructure\Connector\Processor;
...
use Akeneo\Pim\Tailored\Application\ExtractMedia\ExtractMediaQuery;
use Akeneo\Pim\Tailored\Application\ExtractMedia\ExtractMediaQueryHandler;
use Akeneo\Pim\Tailored\Application\MapValues\MapValuesQuery;
use Akeneo\Pim\Tailored\Application\MapValues\MapValuesQueryHandler;
use Akeneo\Pim\Tailored\Infrastructure\Hydrator\ColumnCollectionHydrator;
use Akeneo\Pim\Tailored\Infrastructure\Hydrator\ValueCollectionHydrator;
...
class EntityWithValuesExportProcessor implements ItemProcessorInterface, StepExecutionAwareInterface
{
    ...
    public function process($entity)
    {
        if (
            ...
            && !$entity instanceof ProductModelInterface
        ) {
        ...
        $valueCollection = $this->valueCollectionHydrator->hydrate($entity, $columnCollection);

        $mapValuesQuery = new MapValuesQuery($columnCollection, $valueCollection);
        $mappedEntities = $this->mapValuesQueryHandler->handle($mapValuesQuery);
        ...
        return new ProcessedTailoredExport($mappedEntities, $filesToExport);
    }
