...
const LocaleSwitcher: React.FC<LocaleSwitcherProps> = ({locales, localeCode, onChange}) => {
  ...
  return (
    <Dropdown>
      <SwitcherButton label={translate('pim_enrich.entity.locale.uppercase_label')} onClick={open}>
        <LocaleFlag code={currentLocale.code} languageLabel={currentLocale.language} />
      </SwitcherButton>
      {isOpen && (
        <Dropdown.Overlay verticalPosition='down' onClose={close}>
          ...
          <Dropdown.ItemCollection>
            {locales.map(locale => (
              <Dropdown.Item key={locale.code} onClick={() => handleChange(locale.code)}>
